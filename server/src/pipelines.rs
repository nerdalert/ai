...
pub fn resolve_pipelines(
    ...
    for listener in &config.listeners {
        ...
        validate_grid_provider_boundary(listener, &entries, &chains)?;
...
///
/// `x-grid-peer-*` fields are intentionally outside Praxis's reserved namespace
...
fn validate_grid_provider_boundary(
    listener: &Listener,
    entries: &[FilterEntry],
    ...
    if provider_consumer_exists_in_branch(entries, chains, &mut HashSet::new()) {
        return Err(format!(
            "listener '{}': grid_provider_route must be top-level, not branch-conditional",
            listener.name
        )
        .into());
    }
    ...
    let provider_indices = entries
        .iter()
        .enumerate()
        .filter_map(|(index, entry)| (entry.filter_type == "grid_provider_route").then_some(index))
        .collect::<Vec<_>>();
...
/// Require mutual TLS on a listener that consumes Grid provider context.
fn validate_provider_listener_tls(listener: &Listener) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    {
        return Err(format!(
            "listener '{}': grid_provider_route requires tls.client_cert_mode: require",
            listener.name
        )
        .into());
    }
...
fn validate_provider_entry(
    ...
    let provider = entries
        .get(provider_index)
        .ok_or_else(|| format!("listener '{}': invalid grid_provider_route index", listener.name))?;
...
fn required_peer_identity_trust<'a>(
    ...
    let preceding = entries
        .get(..provider_index)
        .ok_or_else(|| format!("listener '{listener_name}': invalid grid_provider_route prefix"))?;
    let (peer_index, peer) = preceding
        ...
        .ok_or_else(|| {
            format!("listener '{listener_name}': grid_provider_route requires a preceding peer_identity_trust")
        })?;
    if peer_index != 0 {
        return Err(format!(
            "listener '{listener_name}': peer_identity_trust must be the first filter in a Grid provider chain"
        )
        .into());
    }
    Ok(peer)
        ...
        entry.branch_chains.as_ref().is_some_and(|branches| {
            branches.iter().any(|branch| {
                branch.chains.iter().any(|chain| {
                    ...
                    nested.iter().any(|filter| filter.filter_type == "grid_provider_route")
...
fn validate_unconditional_closed(
    ...
    if entry.failure_mode != FailureMode::Closed || !entry.conditions.is_empty() {
        return Err(format!(
            "listener '{listener_name}': {} must be unconditional and fail-closed for the Grid provider boundary",
            entry.filter_type
        )
        .into());
    }
...
"#,
    ...
    #[test]
    fn grid_provider_route_rejects_plaintext_listener() {
        let (listener, entries) = provider_boundary_parts("", peer_then_provider_filters());

        let err = validate_grid_provider_boundary(&listener, &entries, &HashMap::new())
    ...
    #[test]
    fn grid_provider_route_rejects_optional_client_certificate() {
    ...
    #[test]
    fn grid_provider_route_rejects_missing_peer_trust() {
    ...
    #[test]
    fn grid_provider_route_rejects_peer_trust_after_consumer() {
    ...
    #[test]
    fn grid_provider_route_rejects_open_peer_trust() {
    ...
    #[test]
    fn grid_provider_route_rejects_filter_before_peer_trust() {
    ...
    #[test]
    fn grid_provider_route_rejects_open_provider_consumer() {
    ...
    #[test]
    fn grid_provider_route_rejects_conditional_peer_trust() {
        let filters = format!(
            "{}{}",
    ...
    #[test]
    fn grid_provider_route_rejects_conditional_provider_consumer() {
        let provider = provider_filter().replacen(
            "        provider_id:",
            "        conditions:\n          - when:\n              path_prefix: /v1\n        provider_id:",
            1,
    ...
    #[test]
    fn grid_provider_route_rejects_branch_conditional_consumer() {
...
",
    ...
    #[test]
    fn grid_provider_route_accepts_required_mtls_and_preceding_peer_trust() {
        let (listener, entries) = provider_boundary_parts(&provider_tls("require"), peer_then_provider_filters());

        validate_grid_provider_boundary(&listener, &entries, &HashMap::new()).expect("valid provider boundary");
    }
