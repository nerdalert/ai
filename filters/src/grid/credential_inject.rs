...
//! [`GridCredentialInjectFilter`] — injects the credential reference selected
//! by an authorized Grid routing filter into the outgoing request.
//!
//! # Overview
//!
//! `grid_route` or `grid_provider_route` writes a selected credential
...
/// `grid_route` or `grid_provider_route`.
///
/// Reads `grid.route.credential.*` filter metadata written by the preceding
...
impl HttpFilter for GridCredentialInjectFilter {
    ...
    async fn on_request(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        ...
        let header_value = HeaderValue::from_str(cred.header_value.as_str()).map_err(|e| -> FilterError {
            format!("grid_credential_inject: invalid resolved Authorization header: {e}").into()
        })?;
...
fn selected_credential_ref(ctx: &HttpFilterContext<'_>) -> Option<Result<CredentialRef, ()>> {
    ...
    if strategy != Some(STRATEGY_BEARER_TOKEN) || name.is_none() || namespace.is_none() || key.is_none() {
        tracing::debug!(
            ...
            "grid_credential_inject: incomplete or unsupported credential reference; failing closed"
        );
        return Some(Err(()));
    }
    ...
    match (&entry.value, &entry.env_var, &entry.file) {
        ...
        (None, None, Some(path)) => {
            ...
            if content.len() > MAX_TOKEN_BYTES {
                return Err(format!("grid_credential_inject: file '{path}' exceeds {MAX_TOKEN_BYTES} bytes").into());
            }
...
fn validate_bounded(field: &str, value: &str, maximum: usize) -> Result<(), FilterError> {
    if value.trim().is_empty() || value.len() > maximum {
        return Err(format!("grid_credential_inject: {field} must contain 1-{maximum} bytes").into());
    }
    Ok(())
}
