...
//! Provider-local validation and routing for a Grid-selected provider hop.
//!
//! This is intentionally not a second `grid_route`: it does not discover,
...
//! reference for `grid_credential_inject`. Edge peer headers are AI-owned,
...
struct GridProviderRouteConfig {
...
struct ProviderRouteConfig {
    /// Stable candidate ID selected by the edge `grid_route`.
    candidate_id: String,
...
struct ProviderRoute {
    ...
    cluster: Arc<str>,
    /// Optional credential reference for `grid_credential_inject`.
    credential: Option<CandidateCredential>,
    /// Accepted model name.
    model: Arc<str>,
...
/// Exact provider-local mapping from an authenticated Grid selection to a
/// private backend cluster.
///
...
/// consumers.
pub struct GridProviderRouteFilter {
...
impl GridProviderRouteFilter {
    ...
    pub fn from_config(config: &serde_yaml::Value) -> Result<Box<dyn HttpFilter>, FilterError> {
        let cfg: GridProviderRouteConfig = parse_filter_config("grid_provider_route", config)?;
        validate_value("provider_id", &cfg.provider_id)?;
        if cfg.routes.is_empty() || cfg.routes.len() > MAX_PROVIDER_ROUTES {
            return Err(format!("grid_provider_route: routes must contain 1-{MAX_PROVIDER_ROUTES} entries").into());
        }
        ...
        for route in cfg.routes {
            ...
            if routes.insert(candidate_id, provider_route).is_some() {
                return Err("grid_provider_route: duplicate candidate_id".into());
            }
        }
...
#[async_trait]
impl HttpFilter for GridProviderRouteFilter {
...
fn set_provider_headers(
    ...
    ctx.request_headers_to_set.push((
        ...
        HeaderValue::from_str(request_id)
            .map_err(|e| FilterError::from(format!("grid_provider_route: invalid provider request ID: {e}")))?,
    ));
    ctx.request_headers_to_set.push((
...
fn validate_route(route: &ProviderRouteConfig) -> Result<(), FilterError> {
    ...
    if route.paths.is_empty() || route.paths.len() > MAX_PATHS_PER_ROUTE {
        return Err(format!("grid_provider_route: paths must contain 1-{MAX_PATHS_PER_ROUTE} entries").into());
    }
    let mut seen_paths = HashSet::with_capacity(route.paths.len());
    for path in &route.paths {
        if !path.starts_with('/') || path.len() > MAX_VALUE_LEN {
            return Err("grid_provider_route: paths must be absolute and bounded".into());
        }
        if !seen_paths.insert(path) {
            return Err("grid_provider_route: duplicate path".into());
        }
    }
    if let Some(credential) = &route.credential {
        ...
        if credential.strategy != super::metadata::STRATEGY_BEARER_TOKEN {
            return Err("grid_provider_route: unsupported credential strategy".into());
        }
    }
    Ok(())
}
...
fn validate_value(field: &str, value: &str) -> Result<(), FilterError> {
    if value.trim().is_empty() || value.len() > MAX_VALUE_LEN {
        return Err(format!("grid_provider_route: {field} must be non-blank and at most {MAX_VALUE_LEN} bytes").into());
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::{
        grid::{
            credential_inject::GridCredentialInjectFilter,
    ...
    fn make_filter(candidate_id: &str) -> Box<dyn HttpFilter> {
        ...
        GridProviderRouteFilter::from_config(&config).unwrap()
    }
    ...
    #[tokio::test]
    async fn strips_grid_protocol_headers() {
    ...
    async fn credential_route_drives_final_hop_injection() {
        let provider = GridProviderRouteFilter::from_config(&credential_provider_config()).unwrap();
        ...
        .unwrap();
        let inject = GridCredentialInjectFilter::from_config(&inject_config).unwrap();
        let mut req = request_with_peer_context("/v1/chat/completions", "abc123", "req-1", "sim-model-v1");
        req.headers
            .insert(AUTHORIZATION, HeaderValue::from_static("Bearer customer-secret"));
        let mut ctx = test_utils::make_filter_context(&req);
    ...
    fn empty_routes_rejected() {
        ...
        assert!(GridProviderRouteFilter::from_config(&val).is_err());
    }
    ...
    fn duplicate_candidate_id_rejected() {
        let yaml = "provider_id: p\nroutes:\n  - candidate_id: a\n    model: m\n    paths: [/x]\n    cluster: c\n  - candidate_id: a\n    model: m2\n    paths: [/y]\n    cluster: c2\n";
        let val: serde_yaml::Value = serde_yaml::from_str(yaml).unwrap();
        let result = GridProviderRouteFilter::from_config(&val);
    ...
    fn duplicate_path_rejected() {
        ...
        assert!(GridProviderRouteFilter::from_config(&config).is_err());
    }
