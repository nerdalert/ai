...
mod tests {
    ...
    #[test]
    fn parse_overlay_with_credential() {
        let json = r#"{
            "local_site": "site-a",
            "candidates": [{
                "kind": "inference_model",
        ...
        let json = r#"{
            ...
            "candidates": [{
                ...
                "credential": {
                    "strategy": "bearer_token",
                    "token": "sk-1234567890",
                    "secret_ref": { "name": "k", "namespace": "ns", "key": "t" }
                }
            }]
        }"#;
        let result = RouteSnapshot::from_overlay(json.as_bytes());
    ...
    fn retain_on_invalid_credential_fields() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("grid-config.json");
        let json = r#"{"local_site":"a","candidates":[{"kind":"inference_model","name":"m","site":"s","cluster":"c","fresh":true,"credential":{"strategy":"bearer_token","token":"leaked","secret_ref":{"name":"k","namespace":"n","key":"k"}}}]}"#;
        std::fs::write(&path, json).unwrap();
        let (snap, hash) = make_valid_snapshot();
        handle_overlay_reload(&path, &snap);
        assert_eq!(snap.load().content_hash, hash);
