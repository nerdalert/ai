...
//! Local token-rate-limit admission and reconciliation.
//!
...
//! ```
//!
//! Admission is local and synchronous; actual `token.total` usage is consumed
//! from existing token-count metadata during response reconciliation.
...
impl HttpFilter for TokenRateLimitFilter {
    ...
    async fn on_response(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        ...
        } else if let Some(total) = ctx
            .get_metadata(META_TOKEN_TOTAL)
            .and_then(|value| value.parse::<u64>().ok())
        {
            self.reconcile(ctx, Some(total)).await;
        } else if ctx
            .get_metadata(META_RESERVATION_ID)
            .and_then(|value| value.split_once(':'))
            .and_then(|(rule, _)| rule.parse::<usize>().ok())
            .and_then(|index| self.rules.get(index))
            .is_some_and(|rule| rule.backend.local_state().is_some())
        {
            self.reconcile(ctx, None).await;
        }
        Ok(FilterAction::Continue)
...
fn parse_budget(config: &BudgetConfig) -> Result<Budget, FilterError> {
    ...
    Ok(Budget {
        window_ms: parse_duration_ms(&config.window)?,
